...
dict_cogn = {'L.NARTIQ': 'NART Full-Scale IQ',
 'L.GNT': 'Graded Naming Test',
 'L.beachNarr': 'Oz beach scene: total narrative words',
 'L.beachden': 'Oz beach scene: propositional density',
 'L.beachPropV': 'L.beachPropV',
 'L.holidayNarr': 'Best Holiday: total narrative words',
 'L.holidayden': 'Best Holiday: propositional density',
 'L.holiPropV': 'L.holiPropV',
 'L.cookieNarr': 'Cookie Theft: total narrative words',
 'L.cookieden': 'Cookie theft: propositional density',
 'L.cookiePropV': 'L.cookiePropV',
 'L.Animal': 'Animal: total correct',
 'L.m.Ani%Terr': 'Animal: percentage total errors',
 'E.m.StroopDrt': 'Stroop: dot colour naming reaction time',
 'E.m.StroopWrt': 'Stroop: word colour naming reaction time',
 'E.m.StroopCrt': 'Stroop: colour naming reaction time',
 'E.stroopC': 'Stroop: colour naming correct',
 'E.m.DiffBrtArt': 'HSCT: Part B RT and Part A RT difference',
 'E.m.hsctE45': 'HSCT: global error score',
 'E.PropCStrat': 'HSCT: proportion correct using a strategy',
 'E.FAS': 'FAS: total correct',
 'E.m.FAS15:45': 'FAS: proporation of words in first 15 seconds',
 'E.m.FAS%Terr': 'FAS: percentage of total errors',
 'E.DStotal': 'Digit Span (WAIS-IV)',
 'E.m.teaTSA': 'TEA Telephone Search: time per target score',
 'E.m.TEA-E': 'TEA Dual Task: time per target score',
 'S.Emotion': 'Mini-SEA Emotion Recognition',
 'S.TASITB': 'TASIT-S: part B total correct',
 'S.m.HADSa': 'HADS: anxiety',
 'S.m.HADSd': 'HADS: depression',
 'M.RAVLT1': 'RAVLT: list learning trial 1 (List A)',
 'M.RAVLTlearn': 'RAVLT: total learned trial 1 to trial 5',
 'M.m.learnErr': 'RAVLT: total errors trial 1 to trial 5',
 'M.learnRep': 'RAVLT: total repetitions trial 1 to trial 5',
 'M.LOT': 'RAVLT: learning over time',
 'M.RAVLTb': 'RAVLT: list learning trial 1 (List B)',
 'M.RAVLT6': 'RAVLT: immediate free recall (List A)',
 'M.RAVLT7': 'RAVLT: delayed free recall (List A)',
 'M.RAVLTA': 'RAVLT: recognition memory hits (List A)',
 'M.P(A)': 'RAVLT: proportion hits to false positives',
 'M.Topo': 'Recognition memory test: Topography'}
...
df_gen['APOEe4'] = [0 if a in [23, 33] else a for a in df_gen.APOEe4]
...
fig, axes = plt.subplots(3, 4, figsize=(20, 20), sharey='row')
# groups / education
groups=df_gen.Group
score = df_gen.Education
...
for ax, axh, x in zip([axes[0, 0], axes[0, 2]], 
    ...
    for gr, th in zip([[1, 2], [2, 3], [1, 3]], [0, 5, 15]):
        tstat, pval = stats.ttest_ind(x[df_gen.Group == gr[0]],
                                      x[df_gen.Group == gr[1]])
        if pval < 0.05:
            star = '**' if pval < 0.001 else '*'
            ax.plot([gr[0] + 0.25, gr[0] + 0.25, gr[1] - 0.25, gr[1] - 0.75],
                     [th, th, th, th], linewidth=2, color='k')
            ax.text(sum(gr)/2, th+1, star, ha='center', va='bottom', color='k')
    # scatter plot
    x = np.array(x[df_gen.Group == 1])[~pd.isnull(score[df_gen.Group == 1])]
    y = np.array(score[df_gen.Group == 1])[~pd.isnull(score[df_gen.Group == 1])]
    axh.scatter(y, x, marker="o", s=75)
    linreg = stats.linregress(y, x)
    if linreg.pvalue < 0.05:
        axh.plot(y, linreg.intercept + linreg.slope*y, linewidth=4)
axes[0, 2].set_title('Sulcal Width')
axes[0, 0].set_title('Cognitive Scores')
axes[0, 1].set_xlabel('educaion')
...
score = df_gen['Centiloid.1'][df_gen.Group == 1]
for ax, axh, x in zip([axes[1, 0], axes[1, 2]], 
                      [axes[1, 1], axes[1, 3]], 
                      [Ycogn_r[df_gen.Group == 1,0], Xsulci_r[df_gen.Group == 1,0]]):
    # hist
    listg = sorted([g for g in set(groups) if not pd.isnull(g)])
    data = [list(x[groups == g]) for g in listg]
    labels = [dict_label[g] for g in listg]
    colors = [col[g] for g in listg]
    bplot = ax.boxplot(data, notch=True, vert=True,
                        patch_artist=True, whis=100, labels=labels)
    for patch, color in zip(bplot['boxes'], colors):
        patch.set_facecolor(color)
    for gr, th in zip([[1, 2], [2, 3], [1, 3]], [0, 5, 15]):
        tstat, pval = stats.ttest_ind(x[groups == gr[0]],
                                      x[groups == gr[1]])
        if pval < 0.05:
            star = '**' if pval < 0.001 else '*'
            ax.plot([gr[0] + 0.25, gr[0] + 0.25, gr[1] - 0.25, gr[1] - 0.75],
            ...
            ax.text(sum(gr)/2, th+1, star, ha='center', va='bottom', color='k')
    # scatter plot
    x = np.array(x)[~pd.isnull(score)]
    y = np.array(score)[~pd.isnull(score)]
    axh.scatter(y, x, marker="o", s=75)
    linreg = stats.linregress(y, x)
    if linreg.pvalue < 0.05:
        axh.plot(y, linreg.intercept + linreg.slope*y, linewidth=4)
axes[1, 1].set_xlabel('centiloid')
axes[1, 3].set_xlabel('centiloid')
# apoe / prs
groups=df_gen.APOEe4[df_gen.Group == 1]
col={0:'orchid', 4:'lightskyblue'}
dict_label={0:'no APOE ε4', 4:'APOE ε4'}
score = df_gen['PRS_noAPOE'][df_gen.Group == 1]
for ax, axh, x in zip([axes[2, 0], axes[2, 2]], 
    ...
    if linreg.pvalue < 0.05:
        axh.plot(y, linreg.intercept + linreg.slope*y, linewidth=4)
axes[2, 1].set_xlabel('PRS no APOE')
axes[2, 3].set_xlabel('PRS no APOE')
...
# groups = df_gen['Amyloid'][df_gen.Group == 1]
# df = df_gen[['Sex', 'Age', 'Amyloid', 'PRS_noAPOE', 'APOEe4', 'Education', 'Centiloid.1']][df_gen.Group == 1][~pd.isnull(groups)]
# df['Amyloid'] = [1 if a=='Positive' else a for a in df['Amyloid']]
# df['Amyloid'] = [0 if a=='Negative' else a for a in df['Amyloid']]
# df['lv_cogn'] = Ycogn_r[df_gen.Group == 1][~pd.isnull(groups)]
# df['lv_sulci'] = Xsulci_r[df_gen.Group == 1][~pd.isnull(groups)]
# # anc = ancova(data=df, dv='dv', covar=['Age', 'Sex', 'Education'], between='Amyloid')
...
plt.style.use('default')
plt.rcParams.update({'font.size': 25})
fig, axes = plt.subplots(2, 4, figsize=(20, 20), sharey='row')
                         # gridspec_kw={'width_ratios': [4, 2, 4, 2]})
# prs
age=df_gen.Age[df_gen.Group == 1]
score=df_gen.PRS_noAPOE[df_gen.Group == 1]
for ax, axh, x in zip([axes[0, 0], axes[0, 2]], 
                      [axes[0, 1], axes[0, 3]], 
                      [Xsulci_r[df_gen.Group == 1,0], Ycogn_r[df_gen.Group == 1,0]]):
    x = np.array(x)[~pd.isnull(score)]
    y = np.array(age)[~pd.isnull(score)]
    s = np.array(score)[~pd.isnull(score)]
    m = ax.scatter(y, x, marker="o", c=s, s=75)
    axh.scatter(s, x, marker="o", s=75)
    linreg = stats.linregress(s, x)
    if linreg.pvalue < 0.05:
        axh.plot(s, linreg.intercept + linreg.slope*s, linewidth=4)
axes[0, 0].set_title('Sulcal Width')
axes[0, 2].set_title('Cognitive Scores')
# fig.colorbar(m)
# centiloid
age=df_gen.Age[df_gen.Group == 1]
score=df_gen['Centiloid.1'][df_gen.Group == 1]
...
corr = pd.DataFrame(np.corrcoef([np.array(boostx[i]).mean(axis=0)[:, 0] for i in range(3)]),
                    columns=['PISA HC', 'AIBL HC', 'PISA ALL'],
                    index=['PISA HC', 'AIBL HC', 'PISA ALL'])
import seaborn as sns
mask = np.triu(np.ones_like(corr, dtype=bool))
sns.set(font_scale=1.5)
ax = sns.heatmap(corr, square=True, mask=mask, annot=True)
